...
:END:
:LOGBOOK:
- State "TODO"       from              [2025-12-04 Thu 18:28]
:END:
...
Might be wrong with the right youtube series.

* TODO SFINAE
:LOGBOOK:
- State "TODO"       from              [2025-12-06 Sat 10:19]
:END:
...
:LOGBOOK:
- State "TODO"       from              [2025-12-06 Sat 10:21]
:END:
...
:LOGBOOK:
- State "TODO"       from              [2025-12-06 Sat 10:14]
:END:
...
:END:
:LOGBOOK:
- State "TODO"       from              [2025-12-05 Fri 14:23]
:END:
...
:LOGBOOK:
- State "TODO"       from              [2025-12-05 Fri 11:47]
:END:
...
* TODO The billionaire’s typewriter
:PROPERTIES:
:EXPORT_FILE_NAME: testing-new-styles
:END:
:LOGBOOK:
- State "TODO"       from              [2025-12-11 Thu 17:41]
:END:
...
Medium isn’t the only blogging service riding this wave, though so far
it seems to have the biggest surfboard. Others include Svbtle, Postagon,
and Silvrback. They all promote a similarly constrained approach to
design, which is sometimes called minimalist.
...
* TODO Optimizing a Lock-Free Ring Buffer
:PROPERTIES:
:EXPORT_FILE_NAME: single-producer-single-consumer-queue
:END:
:LOGBOOK:
- State "DONE"       from "WAIT"       [2025-12-06 Sat 10:18]
- State "WAIT"       from "DONE"       [2025-12-06 Sat 10:18]
- State "DONE"       from "TODO"       [2025-12-06 Sat 10:18]
...
{{{marginfig(ringbuffer.jpg, **Ring buffer with 32 slots.** The producer
has filled 15 of them\, indicated by blue. The consumer is behind the
producer\, reading data from the slots\, freeing them as it does so.  A
free slot is indicated by orange.)}}}  You may have heard of something
called a circular buffer, or maybe even a cyclic queue.  Both are just
other names for the /ring buffer/, a specialized queue in which a
...
array{{{sidenote(By using `std::array` we are forcing clients to define
the buffer size as `constexpr`.  It's also common to use instead a
`std::vector` to remove that restriction.)}}}
...
We can now implement the ~push~ (or write) operation{{{sidenote(Note how
one item is left unused to indicate that the queue is full\, when
`head_` is one item behind `tail_` the queue is full.)}}}
...
Next we implement the ~pop~ (or read) operation{{{sidenote(Again note that
...
* TODO Deriving Type Erasure
:PROPERTIES:
:EXPORT_FILE_NAME: type-erasure
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-12-04 Thu 18:12]
...
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-12-11 Thu 18:28]
...
{{{marginfig(diagram.png, **Virtual dispatch diagram.**  The method `foo` is declared virtual in `Base` and overridden in `Derived`.  Both classes get a `vtable`\, and each object gets a `vptr` pointing to the corresponding `vtable`.)}}}
...
#+begin_src plantuml :file diagram.png :eval never
  @startuml
  hide empty members
...
free{{{sidenote(As in _freedom_.)}}} software, and a devoted Emacs
user.  I live in sunny Dublin.

* DONE Page Not Found
CLOSED: [2025-12-07 Sun 11:13]
:PROPERTIES:
:EXPORT_FILE_NAME: 404
:EXPORT_HUGO_SECTION: .
:END:
:LOGBOOK:
- State "DONE"       from "TODO"       [2025-12-07 Sun 11:13]
- State "TODO"       from              [2025-12-05 Fri 20:03]
:END:
