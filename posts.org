...
* DONE Devirtualisation and Static Polymorphism
CLOSED: [2025-12-01 Mon 18:52]
:PROPERTIES:
...
Virtual functions are the standard C++ tool for runtime polymorphism.  A
base interface exposes virtual methods that derived classes can
override.  Calls made through ~Base&~ are dispatched to the appropriate
override dynamically.  Under the hood, a virtual table
(~vtable~) of function pointers and a ~vptr~ pointing at that table is
used{{{sidenote(The `vptr` increases object size and can hurt cache
locality.  The `vtable` makes indirect calls harder to predict\, so they
can result in branch mispredictions and the reduced compiled-time
knowledge prevents inlining and other optimizations.)}}}. A virtual call
loads the ~vptr~, looks up the right entry in the ~vtable~, and performs
an indirect call.

#+begin_src plantuml :file vtable.png :results file
  @startuml
...
instead.  This post explores when devirtualisation is possible, what
the potential gains are, and how to write code for the compiler to
optimise this way.
...
A plain, non‑virtual member function:
#+begin_src cpp -n
  struct Base {
    auto foo() -> int;
  };

  auto bar(Base* base) -> int { return base->foo() + 777; }
#+end_src

On x86-64 with GCC and ~-O3~ optimization level, ~bar~ looks like:
#+begin_src asm
  bar(Base*):
          sub     rsp, 8
          call    Base::foo()
          ...
          ret
#+end_src
This is a direct call: the target is known at compile time, so the
compiler can inline it, propagate constants, and generally optimize
across the call boundary.

Now, make ~foo~ virtual:
#+begin_src cpp -n :hl_lines 3
  struct Base {
...
Here =rdi= holds the argument =b=. The compiler has to:
...
#+begin_src cpp
  ...
  auto test(Derived* derived) -> int { return derived->foo() + derived->bar(); }
#+end_src
...
#+begin_src cpp
  ...
  class Base {
   public:
    auto foo() -> int { return 777 + static_cast<Derived*>(this)->bar(); }
  };

  class Derived : public Base<Derived> {
   public:
    auto bar() -> int { return 888; }
  };
...
#+begin_src cpp
  class Base {
   public:
    auto foo(this auto&& self) -> int { return 777 + self.bar(); }
  };
...
This generates essentially the same optimized code as the CRTP version: =foo= is instantiated as if it were a =foo<Derived>= and the call to =bar= is statically resolved and inlined. The key difference is syntactic and structural:
