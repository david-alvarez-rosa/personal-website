...
#+begin_src sh
  $ dig +short A mail.alvarezrosa.com
  213.32.19.229
  $ dig +short AAAA mail.alvarezrosa.com
  2001:41d0:305:2100::febc
  $ dig +short MX alvarezrosa.com
...
#+begin_src conf
  ssl = required
  ssl_cert = </etc/letsencrypt/live/mail.alvarezrosa.com/fullchain.pem
  ssl_key = </etc/letsencrypt/live/mail.alvarezrosa.com/privkey.pem
#+end_src

Enable SMTP submission. Edit ~/etc/postfix/master.cf~ and uncomment the
submission section[fn::Port 587 is for mail submission with STARTTLS. Port
465 (SMTPS) also works, but 587 is the modern standard.]
...
Open ports 587 (SMTP) and 993 (IMAP)
