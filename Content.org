...
* TODO [#B] Implementing a hash map                                 :backlog:
- https://github.com/rigtorp/spartan/blob/master/HashMap.h
...
* DONE [#B] Self-Hosting Behind CGNAT                          :self_hosting:
CLOSED: [2026-09-21 Mon 08:32]
:PROPERTIES:
:EXPORT_FILE_NAME: self-hosting-behind-cgnat
:EXPORT_HUGO_CUSTOM_FRONT_MATTER: :subtitle "Libre software, libre hardware, and my mother's basement." :image "images/self-hosting-behind-cgnat.png"
:END:
...
self-host at home, and be truly /libre/.

In the past, self-hosting was easier.  You just had to open a port on
your router and forward it to any machine at home.[fn::A dynamic DNS
service kept your domain pointing at the right public IP whenever your
ISP rotated it.]  Nowadays, the shortage of IPv4 addresses means routers
...
#+caption: *Topology diagram.*  The homelab is exposed to the Internet through a WireGuard tunnel to a VPS bridge.
#+begin_example -i
  +-------------------------------------+
  |           public Internet           |
  +-------------------------------------+
...
A bidirectional WireGuard tunnel[fn::[[https://www.wireguard.com/][WireGuard]] is a fast, modern and
secure VPN tunnel that lives inside the Linux kernel.] forwards all
...
you don't need a static dedicated IP at home.[fn::Buying a static IP
from your ISP is a valid alternative, at around 20 euros a month in
Spain.]  The penalty of the bridge is 39 ms of RTT.

** Tunnel configuration

Bridge's ~wg0.conf~.[fn::See [[/posts/first-steps-on-a-new-server/][First Steps on a New Server]] for how I set
up a fresh machine.]
...
~PostUp~ sets up NAT and forwarding rules at the kernel level.[fn::And
~PostDown~ removes them when the tunnel goes down.]  The first two
...
Homelab's ~wg0.conf~.[fn::Its full configuration lives in my [[https://github.com/david-alvarez-rosa/homelab][homelab]]
repository.]
...
leaving the homelab's own traffic on the home router.[fn::From then on,
...
- /Homelab./ A cronjob in the homelab checks whether SSH is still
  working and, if it is not, reboots the box.
- /Bridge./ In case it fails, I recommend a backup entry point like a
  Cloudflare tunnel or Tailscale directly to the homelab.
- /Tunnel./ A short drop re-handshakes on its own.  A longer one is
...
* DONE [#B] Optimizing a Spin-Lock                          :cpp:performance:
CLOSED: [2026-08-27 Thu 14:23]
:PROPERTIES:
...
:END:
:LOGBOOK:
CLOCK: [2026-08-08 Sat 12:25]--[2026-09-08 Tue 21:48] => 753:23
...
Threads increment a shared counter under the lock.[fn::Run on a box
[[/posts/tuning-a-server-for-benchmarking/][tuned for benchmarking]].  Built with ~clang~.  All optimizations
enabled.]
...
four, and one branch in eight is mispredicted.[fn::Whether the exchange
...
Spinning costs energy.[fn::High-frequency trading shops care about it.
[[https://www.nyse.com/technology/colo][Exchange colocation services]] charge for power, and NYSE [[https://www.federalregister.gov/documents/2023/11/20/2023-25548/self-regulatory-organizations-new-york-stock-exchange-llc-nyse-american-llc-nysearca-inc-nyse][caps]] at 32 kW.]  At
four threads it draws *64.92 J*.[fn::Reading the RAPL counters requires
...
instruction marks the loop as a spin-wait, so the core idles.[fn::The
...
cap.[fn::Example 2-10, /Contended Locks with Increasing Back-off/, in
the [[https://cdrdv2.intel.com/v1/dl/getContent/671488][Intel Optimization Reference Manual]] (PDF, 248966-050US).]
...
Reproduce it with the [[https://github.com/david-alvarez-rosa/CppPlayground/blob/main/dsa/spin_lock.cpp][benchmark]].
...
measuring.[fn::With one writer and many readers, consider a ~seqlock~
instead.]
...
Works in algorithmic trading at Susquehanna and represents Ireland on
the C++ standards committee.  Previously designed systems at Amazon
serving 10M+ monthly active customers, developed semantic caching for
LLMs at Sopra Steria, and analyzed cybersecurity risk at Deloitte.
...
/Member @ C++ Standards Committee/
[fn::Jul 2026--Present@@html:<span class="sep"></span>@@Remote]\\
National expert.  Representing Ireland.
