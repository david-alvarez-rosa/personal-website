...
* DONE [#B] Self-Hosting an Email Server
CLOSED: [2026-02-03 Mon 21:00]
:PROPERTIES:
:EXPORT_FILE_NAME: selfhost-email-server
:END:

Running your own email server sounds intimidating, but it's more
straightforward than you'd think. After hosting email for multiple
domains, these are the essential steps I follow every time.

This guide assumes you have a server with a clean Linux install[fn::I use
Debian for servers. For initial server setup, see my [[/server-setup/][Server Setup]]
post.] and a domain name pointing to your server's IP.

** DNS configuration

Create DNS records for your mail server: A/AAAA records for
mail.alvarezrosa.com, and an MX record pointing to it.[fn::MX records
...
Set your server's hostname to match. Edit ~/etc/hosts~[fn::Postfix uses
the FQDN (Fully Qualified Domain Name) to identify itself in SMTP
conversations. Make sure ~hostname -f~ returns your mail server's
address.]
...
Check here that hostname -f returns FQDN (but hostname can stay just
homelab).

** Receiving mail

Install Postfix and open port 25[fn::During installation, select "Internet
Site" and enter ~mail.alvarezrosa.com~ as the system mail name. Postfix
handles SMTP---it receives mail from other servers and stores it in
~/var/mail/~.]
...
Restart and verify Postfix is listening[fn::Use ~telnet mail.alvarezrosa.com 25~ to test the
connection. You should see a Postfix greeting.]
...
#+begin_src sh
  $ sudo systemctl restart postfix
  $ echo "Hello, world" | mail -s "Subject" recipient@example.com
#+end_src

If you try to send email to a big email provider like GMail, etc, it
might not arrive at all (not even to Spam folder). That's because they
have strict checks. You can use a recipient something else like???

I receiveda an email back (that's because of DMARC that I setup before...)
#+begin_src text
<davidolazegues@gmail.com>: host gmail-smtp-in.l.google.com[64.233.167.26]
    said: 550-5.7.26 Unauthenticated email from alvarezmagan.com is not
    accepted due to 550-5.7.26 domain's DMARC policy. Please contact the
    administrator of 550-5.7.26 alvarezmagan.com domain if this was a
    legitimate mail. To learn 550-5.7.26 about the DMARC initiative, go to 550
    5.7.26  https://support.google.com/mail/?p=DmarcRejection
    ffacd0b85a97d-43618071e8dsi5766024f8f.211 - gsmtp (in reply to end of DATA
    command)
#+end_src
...
Get a TLS certificate[fn::Certbot obtains free certificates from Let's
Encrypt and auto-renews them. Email clients require TLS.]
...
Also, configure Postfix TLS in /etc/postfix/main.cf
#+begin_src
...
Enable SMTP submission. Edit ~/etc/postfix/master.cf~ and uncomment the
submission section
...
Open ports 465 (SMTP) and 993 (IMAP)
...
Connect Dovecot authentication to Postfix. Edit
~/etc/dovecot/conf.d/10-master.conf~ and find ~service auth~[fn::This
lets Postfix authenticate users against Dovecot, so clients can send mail
using their Linux credentials.]
...
Configure your email client.[fn::I use mu4e in Emacs. For testing,
Thunderbird works well and auto-configures most settings. Use IMAP server
~mail.alvarezrosa.com~ port 993, SMTP server ~mail.alvarezrosa.com~ port
587, both with TLS and your Linux user credentials.] Verify you can send
and receive.
...
*rDNS.* Configure reverse DNS through your VPS provider's control
panel.[fn::Also called PTR records. rDNS maps your IP back to your domain,
proving you control it. Most servers reject mail from IPs without proper
rDNS.] Map your IPs to ~mail.alvarezrosa.com~ and verify
...
*SPF.* Create a DNS TXT record specifying which IPs can send mail for your
domain[fn::SPF prevents spammers from forging your domain. ~v=spf1 mx
-all~ means only servers in your MX records can send; reject all others.
Use ~?all~ for soft fail during testing.]
...
*DKIM.* Install OpenDKIM to sign outgoing messages[fn::DKIM adds a digital
signature verified against a public key in DNS, proving the message came
...
~mail._domainkey.alvarezrosa.com~. The value is in the generated file
...
"v=DKIM1; h=sha256; k=rsa; p=MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEA0xdtbvzBrvRcevo35tuoPo1o3Tx4Tk1kWSYY4VF6NNe1yTCft/+cwt0yu1LgZI8+JBKaKgnGaoXzXUSiDcbCAze1IhKq6ZT1B7IO0bVHVsgtxthAtRotdyqhdq1Ok67MzMs8UfE/EKN69dnEkYZFnJsdbHMxXLWdkb3RECtfe9DHP5uqYzvuvFG" "mfR1wqvv+Z5Dh++9Ncw3Hwqxw3EgRmNaIojTY9e/dQkpae5fBJyFLN0lSKqrTKKX6q5JRGotv4nWHswzW0F31rCdbzBFHO4ZOPgirqiIXnDU9hb38Py9Z6ZPaGpPACFxWpRiTHjdySW0h56RIPxB5Hr6e87YrCQIDAQAB"
#+end_src

Configure OpenDKIM. Edit ~/etc/opendkim.conf~[fn::Mode ~sv~ tells OpenDKIM
to sign outgoing mail and verify incoming signatures.]
...
*DMARC.* Create a DNS TXT record at ~_dmarc.alvarezrosa.com~ through your
DNS provider's control panel[fn::DMARC ties SPF and DKIM together.
~p=reject~ tells receivers to reject mail that fails. Start with ~p=none~
to monitor without affecting delivery, then switch to ~p=reject~ once
everything works. The ~rua~ parameter sends aggregate reports to your
email.]
...
Send a test email to Gmail and inspect the headers---SPF, DKIM, and DMARC
should all pass. Use [[https://www.mail-tester.com][mail-tester.com]] for a comprehensive check (aim for
10/10) and [[https://mxtoolbox.com/SuperTool.aspx][MX Toolbox]] to verify DNS records.

Your email server is now fully operational. Messages should reach inboxes
instead of spam folders.[fn::New mail servers often face deliverability
issues due to IP reputation. If your mail lands in spam initially, keep
sending legitimate emails and request delisting from any blacklists your
IP appears on. It can take weeks to build a positive reputation.]
