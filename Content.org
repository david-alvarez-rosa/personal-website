...
* TODO [#B] Initial server setup
1. Get a distro (LTS recommended, Debian 13) and a domain name. If it's
   own hardware, install the server. For partitions, it's very difficult
   to tell, so I prefer to just have one big ~/~ + swap
2. Point domain to VPS IP from DNS provider, wait until changes are
   propagated through global CDN. An A record for IPv4 and an AAAA
   record for IPv6.
   #+begin_src sh
    $ dig alvarezmagan.com A +short
    213.32.19.229
    $ dig alvarezmagan.com AAAA +short
    2001:41d0:305:2100::febc
  #+end_src
4. Log-in with provided credentials and change root password
   #+begin_src
     $ ssh root@alvarezmagan.com
     # passwd
   #+end_src
   Validate that new password is working
5. Create new user ~david~ with sudo and password
   #+begin_src
     # useradd --create-home --groups sudo david
     # passwd david
   #+end_src
   Validate new password an user with ~su david~
6. Log out, log back with user
   #+begin_src sh
     $ ssh david@alvarezmagan.com
   #+end_src
7. Upgrade and update
   #+begin_src
     ...
     # apt upgrade
   #+end_src
8. Get dotfiles (optionally setup github with ssh keys)
    #+begin_src sh
      $ git init
      $ git remote add origin git@github.com:david-alvarez-rosa/dotfiles.git
      $ git fetch origin
      $ git checkout -t origin/main
      $ git submodule update --init --recursive
      $ git config status.showUntrackedFiles no
   #+end_src
9. Configure shell (install zsh, oh-my-zsh and plugins and starship)
   #+BEGIN_SRC
   ...
   #+end_src
   Relogin and validate that new shell is open with no errors.
10. Configure ssh keys for sshing without password (from local), also for root
    #+begin_src sh
      $ ssh-copy-id david@alvarezmagan.com
      $ ssh david@alvarezmagan.com
      $ su -
      $ cp /home/david/.ssh/authorized_keys ~/.ssh/authorized_keys
    #+end_src
    Validate that you can loging to root and david without password.  I
    like to keep password authentication enabled for user, but disabled
    for root, but tweak sshd config as per your preference
11. Setup firewall blocking all incoming connections except 22/tcp for
    SSH (safest is to block all, and then allow as per your needs)
    #+begin_src
      # apt install ufw
...
12. Setup fail2ban, a security tool that automatically blocks (“bans”)
    IP addresses that show malicious behavior in your server logs—most
    commonly repeated failed login attempts against SSH.
    #+begin_src
      # apt install fail2ban
      # systemctl enable --now fail2ban
    #+end_src
    Check with systemctl status fail2ban
13. Enable automatic security updates unattended-upgrades
   #+begin_src
     # apt install unattended-upgrades apt-listchanges
     # dpkg-reconfigure --priority=low unattended-upgrades
   #+end_src
14. Setup correct timezone
    #+begin_src
...
1. Install a webserver like nginx
    #+begin_src
      # apt install nginx
      # systemctl enable --now nginx
      # ufw allow 80/tcp
    #+end_src
    Check that HTTP(s) ports are allowed with sudo ufw status, and that
    nginx is up with systemctl status nginx. Then either use curl
    localhost, or navigate with a browser to http://alvarezmagan.com and
    check you see the welcome message
2. Setup TLS/HTTPS
    #+begin_src
      # ufw allow 443/tcp
