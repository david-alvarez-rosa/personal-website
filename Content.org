...
* TODO [#B] Fundamental Theorem of Calculus
:PROPERTIES:
...
finite.] function \(f:[a,b]\to\mathbb{R}\), we can approximate the area
...
so they contain their \(\inf\) and \(\sup\), which are finite.], \(g\)
...
\(F(x)=x^3/3\).  No partitions needed.]  The Lebesgue integral pushes
