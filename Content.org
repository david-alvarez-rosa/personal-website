...
If you want to reproduce these results, run the included [[/code/spsc-bench.cpp][benchmark]]
...
a virtual table (~vtable~) is created /per each class/, and a pointer
...
* Footnotes
[fn:25] The producer uses ~release~ so the consumer sees the data before
