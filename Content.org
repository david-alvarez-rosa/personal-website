...
servers.  For initial server setup, see my [[/posts/initial-server-setup/][Server Setup]] post.] and a
...
:PROPERTIES:
:EXPORT_FILE_NAME: initial-server-setup
:END:
...
Log out and reconnect as the new user.[fn::From this point forward,
...
misconfiguration.[fn::The commands below treat your home directory as a
...
root.[fn::Sometimes I SSH from machines without my keys.  To disable
...
need.[fn::Open port 22 before enabling the firewall, or you'll lock
yourself out.]
...
intervention[fn::On Debian, unattended-upgrades installs security
...
patterns[fn::fail2ban watches authentication logs and temporarily blocks
...
Install a web server to verify everything works.[fn::nginx is a
...
Secure the site with HTTPS using Let's Encrypt.[fn::Certbot obtains free
