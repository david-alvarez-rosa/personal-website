...
a time, so the damage does not compound.]  The number to beat is *4.44
ms* at four threads.
...
At one thread it beats the baseline by 1.3x.[fn::Locking is now one
atomic instruction, with no library code around it.]  At two threads it
...
one in ten branches is mispredicted.[fn::Whether the exchange succeeds
...
[[https://www.nyse.com/technology/colo][Exchange colocation services]] bill power, and NYSE [[https://www.federalregister.gov/documents/2023/11/20/2023-25548/self-regulatory-organizations-new-york-stock-exchange-llc-nyse-american-llc-nysearca-inc-nyse][caps]] at 32 kW.]  At
four threads it draws *49.23 J* against the baseline's 32.98 J for the
same work.[fn::Reading the RAPL counters requires system-wide mode
...
backoff.[fn::~volatile~ keeps the compiler from deleting the empty loop.
...
Instructions jump from 2 to 28 billion for the same work.[fn::The delay
...
unpredictable, just outnumbered.]  The extra ones increment the
~volatile~ counter.
...
~_mm_pause~.[fn::Declared in ~<xmmintrin.h>~, which ~<immintrin.h>~
pulls in.]  It marks the loop as a spin-wait, so the core idles.
...
spin-lock only when the threads have dedicated cores,[fn::The standard
...
hands the core back instead.] and you have measured the difference.
