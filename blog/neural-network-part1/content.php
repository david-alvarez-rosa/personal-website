...
<section class="hidden">
   ...
   <p>
      ...
      adjacent layers are required to be connected (and weighted). In the <a
      href="#fig:neural-network">figure 1</a> below, is shown an example of a
   ...
   <p>
      ...
      recognition problem. Suppose we have a set of images with handwritten
      digits in it and that we will like to implement an ANN that is capable of
      ...
      with an image holding a handwritten number 3, then the ouput is one 1 in
      the position corresponding to the number 3 and the rest of zeros.
   </p>
   ...
   <ul>
      <li>
         <strong>Value</strong>: each neuron holds a value, it will be denoted
         by $x_i^l \in \mathbb{R}$ for the $i$-th neuron in the $l$-th layer. Of
         course, it should be satisfied $1 \leq i \leq n_l$. We will use use the
         notation $\mathbf{x}^l$ for the <strong>vector</strong> of all the
         values in the $l$-th level.  When we speak of the input vector, we may
         ommit the superindex, i.e. we will use $\mathbf{x}$ to denote
         $\mathbf{x}^0$. Similarly, for the output layer, we will use
         $\mathbf{\hat{y}}$ to refer to $\mathbf{x}^L$.
      </li>
      <li>
         <strong>Bias</strong>: also each neuron has a bias, denoted as $b_i^l$
         for the $i$-th neuron in the $l$-th layer. Is then true that $1 \leq i
         \leq n_l$. The vector of all biases in the $l$-th layer will be denoted
         by $\mathbf{b}^l$.
      </li>
      <li>
         <strong>Activation function:</strong> all neurons have an activation
         function $f_i^l \in \mathcal{C}^1(\mathbb{R}, \mathbb{R})$ for the
         $i$-th neuron in the $l$-th layer. Of course, it is needed $1 \leq i
         \leq n_l$.
         <span id="callOutFunctions" class="callOut">
            ...
            <span>
               <p>
                  ...
                  <strong>neuron-independent</strong> (i.e. $f_i^l$ does not
   ...
   <p>
      ...
      the connections, that should have associated a
      <strong>weight</strong>. For the connection between the $i$-th neuron in
      the $l$-th layer and the $j$-th neuron in the $l+1$-th layer, we will
      denote this weight by $w_{ij}^l \in \mathbb{R} $. The set of all this
      weights is as follows, \[ \{w_{ij}^l \mid 1 \leq i \leq n_{l}, \, 1 \leq j
      \leq n_{l+1}, \, 1 \leq l < L \} \subset \mathbb{R}. \]
   </p>
   <p>
...
<section class="hidden">
   ...
   <p>
      ...
      forward propagation. It's important to note that it's a recursive formula,
      once the values the neurons in the the input layer are known, we can
      iterate computing the values of the neurons in the the next adjacent
      layer, until we reach the output layer. In this network we can think of
      <em>information</em> travelling in one direction, forward, from the input
      layer, through the hidden layers to the output neurons.
   </p>
</section>
...
<section class="hidden">
   ...
   <ul>
      <li>
         ...
         denote the size (cardinal) of the <strong>dataset</strong> by $N$. Of
         course, in terms of our ANN $\mathbf{x}$ corresponds to the values of
         the first (or input) layer and $\mathbf{y}$ to the output (or last)
         layer.
      </li>
      <li>
   ...
   <p>
      ...
      $\mathbf{\hat{y}}$ of the neural network on input $\mathbf{x}$ for a set
      of input-output pairs $(\mathbf{x}, \mathbf{y}) \in \Omega$ and a
      particular value of the parameters $\mathbf{\theta}$. We will denote the
      error funcion by $E(\Omega, \theta)$ and we will assume that is
